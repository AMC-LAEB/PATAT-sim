...
cdef class Simulation:
    ...
    cdef int32 pcr_ct_thres
    cdef int8 ignore_test_specificity, pcr_test_bool #, confirm_with_pcr_bool

    # isolation and quarantine
    cdef int8 quarantine_hcf_bool, quarantine_social_bool, daily_test_quarantine_bool
    ...
    cdef np.ndarray total_symp_testing_results, total_community_testing_results, total_exit_testing_results, total_daily_quarantine_testing_results, setting_incidence_arr, simpop_day_of_symptom_onset, length_of_infectious_period, untested_non_covid_symp_lack_of_test
    cdef object curr_seird_arr, simpop_infection_status, simpop_disease_severity, simpop_ct_arr, curr_isoquar_arr, agents_to_quarantine, daily_test_quarantine_agents, curr_days_in_isoquar, simpop_isoquar_arr, simpop_postest_setting,  border_crossing_stats, vtype_infector_to_infectee, untested_covid_symp_lack_of_test_arr, reported_daily_case_arr
    cdef np.ndarray exposed_day_infectee, setting_infectee
    cdef int32 agent_to_track

    # tests allocation
    cdef int8 carry_forward_unused_symp_test_bool
    cdef int32 number_of_rdt_per_100k_per_day, curr_number_of_hcf_rdt
    ...
    cdef object hcf_contact_layer_arr
    ...
    def __init__(self, object individuals_df, object household_contact_layer_arr, object entity_type_to_ids, object social_contact_layer_arr, object master_school_dict, **kwargs):
        ...
        self.f_mutant_severe_prob = kwargs.get("f_mutant_severe_prob", 1.64) # fold change to severe probability if infected by mutant
        ...
        """# confirm with PCR at HCF
        self.confirm_with_pcr_bool = kwargs.get('confirm_with_pcr_bool', 0)"""
        ...
        # carry forward unused tests meant for symptomatic testing (only used in fixed allocation of symp/comm testing)
        self.carry_forward_unused_symp_test_bool = kwargs.get('carry_forward_unused_symp_test_bool', 1)
    ...
    def assign_infection_vars_to_exposed_array(self, np.ndarray agents_arr, np.ndarray infector_arr, int32 day, Social_Entity setting, int8 root_virus_type=-1, object initialise_as=None):
        ...
        for virus_type in np.arange(2):
            ...
            if virus_type > 0:
                ...
                self.var_cross_immunity_arr[1, vtype_agents_arr] = 1 - self.cross_immunity
    ...
    def initialise_infections(self):
        ...
        #np.random.seed(1) # seed here to ensure all initial infected agents are the same
        cdef np.ndarray infected_agents = np.random.choice(self.population_arr, n_infected_wt, replace=False)
        # randomly select immune agents without replacement
        cdef np.ndarray immune_agents = np.random.choice(np.setdiff1d(self.population_arr, infected_agents), n_immune, replace=False)
        ...
        # change variant susceptibilty for immune agent
    ...
    def introduce_mt_infections(self, int32 day): # by exposure
        ...
        cdef np.ndarray infected_agent_ids = np.random.choice(mt_susceptible_arr, n_infected_mt, replace=False)
    ...
    def isoquar_agent_by_pos_test_at_hcf(self, np.ndarray agents_arr, int32 day, int8 hospitalised_bool=0):
        ...
        if self.quarantine_hcf_bool > 0:
            ...
            immd_close_contacts_arr = self.household_contact_layer_arr[self.pmap_households[agents_arr]].tocoo().col
    ...
    def review_disease_progression(self, int32 day):
        ...
        cdef np.ndarray rec_dead_agents, infected_agents, mild_infected_agents, subset_agents, new_status_arr
        ...
        for virus_type, status in zip([0, 1], [infected_wt, infected_mt]):
            ...
            # assumption: a symptomatic person who would test a HCF around symptom onset
            if len(mild_infected_agents) > 0:
                ...
                if len(subset_agents) > 0:
                    ...
                    self.hcf_sample_collection_day_arr[virus_type, subset_agents[~mask]] = -1
        ...
        # return if no symptomatic agents will visit HCF for testing
        if len(symp_agents_who_will_visit_hcf) == 0:
            return

        ##### symptomatic testing #####
        cdef np.ndarray vtype_arr, testing_results, postest_agents, negtest_agents, untested_agents_lack_of_test, compliant_agents, noncompliant_agents, agents_not_getting_test, untested_agents_vtype, all_agents_who_want_test
        ...
        # if there are limited number of tests available for HCF and
        if self.curr_number_of_hcf_rdt > -1:
            if self.curr_number_of_hcf_rdt == 0: # no more tests available this week
                ...
                if (len(symp_agents_who_will_visit_hcf) + len(non_covid_test_agents)) > self.curr_number_of_hcf_rdt:
                    # more agents who want to be tested than there are available tests currently

                    # randomly select agents who won't get tested
                    all_agents_who_want_test = np.union1d(symp_agents_who_will_visit_hcf, non_covid_test_agents)
                    np.random.shuffle(all_agents_who_want_test)
                    agents_not_getting_test = np.random.choice(all_agents_who_want_test, len(symp_agents_who_will_visit_hcf) + len(non_covid_test_agents) - self.curr_number_of_hcf_rdt, replace=False)

                    # randomly select covid-infected agents who won't get tested
                    ...
                    # randomly select non-covid agents who won't get tested
                    ...
                    # empty out all available tests for HCF testing
                    self.curr_number_of_hcf_rdt = 0

                else:
                    # more than enough tests for everyone who wanted to get symptomatically tested today
                    untested_agents_lack_of_test = np.array([])
                    untested_agents_vtype = np.array([])

                    self.curr_number_of_hcf_rdt -= (len(symp_agents_who_will_visit_hcf) + len(non_covid_test_agents))
            ...
            '''print ('untested_non_covid_n', self.untested_non_covid_symp_lack_of_test[day])
            print ('untested_covid_n', len(untested_agents_lack_of_test))'''

        #print (len(symp_agents_who_will_visit_hcf), len(non_covid_test_agents), self.curr_number_of_hcf_rdt)
        ...
        if len(symp_agents_who_will_visit_hcf) > 0:
            ...
            if len(postest_agents) > 0:
                ...
                # noncompliant positively tested agents will not visit HCF/isolate themselves after
                noncompliant_agents = postest_agents[~mask]
        ...
        cdef np.ndarray isoquar_period_arr, immd_close_contacts_arr, entity_id_arr, social_contacts_arr
        if len(non_covid_test_agents) > 0:
            ...
            if len(postest_agents) > 0:
                ...
                if len(postest_agents) > 0:
                    ...
                    mask = self.curr_isoquar_arr[quarantine, postest_agents].toarray()[0] > 0
                    quarantined_agents = postest_agents[mask]
                    if len(quarantined_agents) > 0:
                        # complete their quarantine status
                        self.complete_isoquar(quarantined_agents, quarantine, day, isoquar_reason='complete_quar_to_start_posiso')
                    ...
                    self.start_isoquar(postest_agents, day, isoquar_period_arr, isolation, isoquar_reason='noncovid_symp_testing')
    ...
    def assign_hcf(self,):
        ...
        cdef np.ndarray hcf_arr = np.linspace(0, len(household_arr), hcf_n+1).astype(np.int32)
        ...
        self.hcf_contact_layer_arr = sparse.coo_matrix((data, (row_ind, col_ind)), shape=(hcf_n, self.pop_size)).tocsr()
    ...
    def execute(self, int32 total_days, object start_date='2021-01-01', int8 verbose=1):
        ...
        # ouput array saving when agent was tested positive and where (setting + 1, -1 = symptomatic testing, -2 = noncovid symptomatic testing)
        ...
        self.assign_hcf()
        #print (self.voluntary_hcf_visit_prob)
        ...
        # compute number of available tests per week
        cdef int8 fixed_allotment_strategy_bool = 0
        cdef int32 number_of_rdt_per_week, number_of_hcf_rdt_per_week, n_tests_to_allot
        cdef int32 unused_symp_tests_last_week = 0

        cdef object number_of_comm_rdt_per_week = {}

        if self.number_of_rdt_per_100k_per_day >= 0:
            number_of_rdt_per_week = np.around(self.number_of_rdt_per_100k_per_day * 7 * self.pop_size/1e5).astype(np.int32)
            ...
            if self.prop_rdt_hcf_allocation > -1:
                # boolean that we are using fixed allotment strategy
                fixed_allotment_strategy_bool = 1

                # compute number of tests available for HCF
                number_of_hcf_rdt_per_week = np.around(self.prop_rdt_hcf_allocation * number_of_rdt_per_week).astype(np.int32)

                # compute number of tests available for each community setting under fixed allotment strategy
                number_of_rdt_per_week -= number_of_hcf_rdt_per_week
                for setting in [overseas, household, school_class, workplace_formal, community, church, bars]:
                    if setting in self.dist_comm_rdt_allocation:
                        number_of_comm_rdt_per_week[setting] = np.around(self.dist_comm_rdt_allocation[setting] * number_of_rdt_per_week).astype(np.int32)

                print ('Number of tests available per week: %i (community), %i (healthcare facilities)...'%(number_of_rdt_per_week, number_of_hcf_rdt_per_week))

            else:
                # 2) All tests go to symptomatic testing first, remaining goes to community testing next week
                print ("All %i tests available per week goes to healtcare facilities first..."%(number_of_rdt_per_week))
                number_of_hcf_rdt_per_week = number_of_rdt_per_week
                for setting in [overseas, household, school_class, workplace_formal, community, church, bars]:
                    if setting in self.dist_comm_rdt_allocation:
                        number_of_comm_rdt_per_week[setting] = 0

            # initialise number of available tests at the start of the simulation
            # compute number of tests remaining for HCF at the start of simulation
            self.curr_number_of_hcf_rdt = np.around((1 - (date_range[0].weekday()/7)) * number_of_hcf_rdt_per_week).astype(np.int32)

            # compute number of tests available to community screening at the start of simulation
            self.curr_number_of_comm_rdt = {}
            for setting in [overseas, household, school_class, workplace_formal, community, church, bars]:
                if setting in self.dist_comm_rdt_allocation:
                    self.curr_number_of_comm_rdt[setting] = np.around((1 - (date_range[0].weekday()/7)) * number_of_comm_rdt_per_week[setting]).astype(np.int32)
        ...
        for day, day_obj in enumerate(date_range):
            ...
            if weekday == 0:
                ...
                # refresh available tests on Monday
                if self.curr_number_of_hcf_rdt > -1:
                    self.curr_number_of_hcf_rdt = number_of_hcf_rdt_per_week

                    if self.carry_forward_unused_symp_test_bool > 0:
                        self.curr_number_of_hcf_rdt += unused_symp_tests_last_week

                    for setting in [overseas, household, school_class, workplace_formal, community, church, bars]:
                        if setting in self.dist_comm_rdt_allocation:
                            self.curr_number_of_comm_rdt[setting] += number_of_comm_rdt_per_week[setting]

                print ('SOW_SYMP_TEST:', self.curr_number_of_hcf_rdt, 'SOW_COMM_TEST:', self.curr_number_of_comm_rdt)
            ...
            self.review_disease_progression(day)
            # quarantine agents
            ...
            if verbose == 2: # 2nd level verbosity
                ...
                print ('EOD_SYMP_TEST:', self.curr_number_of_hcf_rdt, 'EOD_COMM_TEST:', self.curr_number_of_comm_rdt, '\n')
            ...
            if weekday == 6:
                ...
                # save number of unused tests meant for symptomatic testing
                unused_symp_tests_last_week = self.curr_number_of_hcf_rdt

                if fixed_allotment_strategy_bool < 1: # if NOT fixed allotment strategy of tests
                    # all unused symp tests last week go to next week's community screening
                    for setting in [overseas, household, school_class, workplace_formal, community, church, bars]:
                        if setting in self.dist_comm_rdt_allocation:
                            n_tests_to_allot = np.around(self.dist_comm_rdt_allocation[setting] * unused_symp_tests_last_week).astype(np.int32)
                            if n_tests_to_allot < self.curr_number_of_hcf_rdt:
                                number_of_comm_rdt_per_week[setting] = n_tests_to_allot
                                self.curr_number_of_hcf_rdt -= n_tests_to_allot # deduct from hcf account
                            else:
                                number_of_comm_rdt_per_week[setting] = self.curr_number_of_hcf_rdt
                                self.curr_number_of_hcf_rdt = 0
                    unused_symp_tests_last_week = self.curr_number_of_hcf_rdt

            '''# epidemic died
            if self.curr_seird_arr[exposed_wt:recovered_wt,:].sum() == 0:
                print ("Epidemic died")
                break'''

        return weekdays_arr, self.pmap_age, epi_seird_arr, epi_isoquar_arr, self.Reff_arr, self.asymp_infector_arr, self.setting_incidence_arr, self.exposed_day_infectee, self.simpop_day_of_symptom_onset, self.length_of_infectious_period, self.setting_infectee, self.hcf_sample_collection_day_arr, self.untested_non_covid_symp_lack_of_test, self.total_symp_testing_results, self.total_community_testing_results, self.total_exit_testing_results, self.total_daily_quarantine_testing_results, eod_test_arr, self.reported_daily_case_arr, self.untested_covid_symp_lack_of_test_arr, self.vtype_infector_to_infectee, self.simpop_infection_status, self.simpop_disease_severity, self.simpop_ct_arr, self.simpop_postest_setting, self.simpop_isoquar_arr, self.border_crossing_stats, self.simpop_travel_days, self.prev_tested_entities, self.hcf_contact_layer_arr
